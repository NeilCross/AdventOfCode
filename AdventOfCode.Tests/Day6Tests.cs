namespace AdventOfCode.Tests
{
    ...
    public class Day6Tests
    {
        ...
        public void Setup()
        {
            this.sut = new Day6(1000, 1000);

            #region input = test data;
            var input = @"toggle 461,550 through 564,900
...
turn on 222,12 through 856,241";
            ...
            this.puzzleInput = input.Split(new[] { '\n' });
        }
        ...
        public void Part1Examples()
        {
            ...
            sut.ProcessingInstructionPart1("turn on", 0, 0, 2, 2);
            Assert.AreEqual(9, sut.TotalLights);

            sut.ProcessingInstructionPart1("toggle", 0, 0, 9, 9);
            Assert.AreEqual(91, sut.TotalLights);

            sut.ProcessingInstructionPart1("turn off", 8, 8, 9, 9);
            Assert.AreEqual(87, sut.TotalLights);
        }

        [Test]
        public void Part1Result()
        {
            sut.ProcessInstructionsPart1(this.puzzleInput);
            var result = sut.TotalLights;
        ...
        {
            sut.ProcessingInstructionPart2("turn on", 0, 0, 0, 0);
            Assert.AreEqual(1, sut.TotalBrightness);

            sut.ProcessingInstructionPart2("toggle", 0, 0, 999, 999);
            Assert.AreEqual(2000001, sut.TotalBrightness);
        }

        [Test]
        public void Part2Result()
        {
            sut.ProcessInstructionsPart2(this.puzzleInput);
            var result = sut.TotalBrightness;
            Trace.WriteLine("Result is " + result);
        }
    }
}
