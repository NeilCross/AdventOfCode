...
//            var parsed = input.Split(new[] { '\n' }).Select(i => i.Split('x').Select(d => int.Parse(d)));
//            //var input = new List<Tuple<int, int, int>>{new Tuple<int, int, int> (1,1,10)};

//            ribbon(parsed).Dump();
//        }

        int wrap(IEnumerable<IEnumerable<int>> input)
        {
            int total = 0;
        ...
        int[] getSmallest(IEnumerable<int> item)
        {
        ...
        int ribbon(IEnumerable<IEnumerable<int>> input)
        {
            int total = 0;
            foreach (var item in input)
            {
